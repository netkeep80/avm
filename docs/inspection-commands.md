...
The only string dispatch occurs in the parser. Once parsing succeeds, execution dispatch is by C++ command type. Runtime relation dispatch remains the existing LinkId-based `Executor` path.
...
Command execution returns typed result variants for link inspection, pair lookup, adjacency, relation query/decode, function/frame inspection, execute and trace operations.
