...
| `CASE-MISSING-REFERENCE` | deterministic failure | exact typed unresolved textual reference before materialization | 🚧 #205 |

Таблица означает **доказанный semantic migration corpus**, а не полную совместимость со всем историческим `base.rm.h`.
...
Во время migration был обнаружен real gap canonical core: existing `if_relation` был lazy, но nested condition/branch выполнялись context-free и теряли `SemanticContextView`.
...
и наличие diagnostic, содержащего exact source marker. Exact exception/JSON wording не является contract.
...
### Принятая граница failure

Для exact frozen marker используется typed adapter failure **до** Native document/projection/execution:

```text
legacy {$ref:"__avm_missing_reference_oracle__"}
 -> semantic adapter recognizes exact frozen unresolved reference
...
### Evidence boundary для других имён

Без caller-owned resolver и frozen success evidence нельзя честно утверждать semantics произвольного standalone `$ref`.

Поэтому #205 deliberately различает:

```text
exact __avm_missing_reference_oracle__
 -> UnresolvedReference

another_unproven_name
 -> InvalidSource / unsupported without frozen evidence
```

Так typed compatibility claim остаётся ровно в границах oracle corpus.
...
B. caller уже supplied canonical LinkId anchor,
...
Поэтому #205 не вводит premature callback/global resolver surface. Когда появится реальный success fixture/consumer, caller-owned textual-name resolver можно добавить evidence-driven, всё так же без доступа migrator-а к LinkStore и без hidden DB lookup.
...
Failure #205 не создаёт `MigrationResult`: unresolved frozen source reference прекращает migration до Native document.
...
Differential test фиксирует `store.size()` до и после failure.
...
- arbitrary unproven standalone name;
...
15. arbitrary unproven name не получает compatibility semantics;
